...
namespace YiAdmin01.BLL.Business
{
    ...
    {
        private DataDictService dataDictService = new();
        private DataDictDetailService dataDictDetailService = new();

        private DataDictCache dataDictCache = new();
        private DataDictDetailCache dataDictDetailCache = new();

        #region 获取数据
        public async Task<TData<List<DataDictEntity>>> GetList(DataDictListParam param)
        {
            TData<List<DataDictEntity>> obj = new();
        ...
        {
            TData<List<DataDictInfo>> obj = new();
            List<DataDictInfo> dataDictInfoList = new();

            List<DataDictEntity> dataDictList = await dataDictCache.GetList();
            var dataDictDetailList = await dataDictDetailCache.GetList();

            foreach (DataDictEntity dataDict in dataDictList)
            {
                List<DataDictDetailInfo> dataDictDetailInfos = dataDictDetailList.Where(p => p.DictType == dataDict.DictType).OrderBy(p => p.DictSort).Select(p => new DataDictDetailInfo
                {
                    DictKey = p.DictKey,
