...
namespace YiAdmin01.BLL.Services.SystemManage
{
    ...
    {
        #region 获取数据
        public async Task<List<SqlTableInfo>> GetTableList(string tableName)
        {
            StringBuilder strSql = new StringBuilder();
            strSql.Append(@"SELECT id Id,name TableName FROM sysobjects WHERE xtype = 'u' order by name");
            var list = await this.BaseRepository().FindList<SqlTableInfo>(strSql.ToString());
        ...
        public async Task<List<SqlTableInfo>> GetTablePageList(string tableName, Pagination pagination)
        {
            StringBuilder strSql = new StringBuilder();
            ...
            IEnumerable < SqlTableInfo> list = await this.BaseRepository().FindList<SqlTableInfo>(strSql.ToString(), parameter.ToArray(), pagination);
            await SetTableDetail(list);
            return list.ToList();
        }
        ...
        /// </summary>
        /// <returns></returns>
        private async Task<List<SqlTableInfo>> GetTableDetailList()
        {
            ...
            IEnumerable<SqlTableInfo> list = await this.BaseRepository().FindList<SqlTableInfo>(strSql.ToString());
            return list.ToList();
        }
        ...
        private async Task SetTableDetail(IEnumerable<SqlTableInfo> list)
        {
            List<SqlTableInfo> detailList = await GetTableDetailList();
            foreach (   SqlTableInfo table in list)
            {
